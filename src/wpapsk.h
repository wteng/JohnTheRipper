...
static void *get_binary(char *ciphertext)
{
	...
	s = mem_alloc_tiny(data.size, MEM_ALIGN_WORD);
	data.blob = s;
...
static void set_salt(void *salt)
{
	...
	if (hccap.keyver == 0) {
		...
		insert_mac(cur_data.data);
		insert_nonce(cur_data.data + 12);
		if (hccap.keyver < 3)
			alter_endianity(cur_data.data, 64 + 12);
	}

	HANDLE_CLERROR(clEnqueueWriteBuffer(queue[gpu_id], mem_salt, CL_FALSE, 0, sizeof(wpapsk_data), &cur_data, 0, NULL, NULL), "Copy setting to gpu");
...
static int cmp_all(void *cur_data, int count)
{
	fmt_data *blob = cur_data;
...
static int cmp_one(void *cur_data, int index)
{
	fmt_data *blob = cur_data;
	hccap_t *hccap = blob->blob;
...
static int salt_hash(void *salt_s)
{
	...
	//fprintf(stderr, "salt %s hash 0x%05x\n", salt->essid, hash & (SALT_HASH_SIZE - 1));
...
static int salt_compare(const void *x, const void *y)
{
	...
	int c = strncmp((const char*)s1->essid, (const char*)s2->essid,
...
#if 0
/* Sadly, this broke when we started using only the ESSID as salt */
